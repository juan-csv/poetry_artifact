...
# args for the DAG
DAG_ID = "dag_ml_life_cycle_docker"
...
with DAG(
    ...
    task_processing = DockerOperator(
        task_id="get_gold_data_DockerPodOperator_docker",
        image='gcr.io/testingv0001/worker_airflow',
        command="python airflow/dags/lifecycle_ml_example/src/features/make_training.py",
    )

    # start task with dummy operator
    start = DummyOperator(task_id="star_docker")
    ...
    end = DummyOperator(task_id="end_docker")

    # define the order of the tasks
    start >> task_processing >> end
